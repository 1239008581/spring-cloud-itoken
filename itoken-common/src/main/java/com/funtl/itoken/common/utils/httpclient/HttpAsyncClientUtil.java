...
/**
 * 使用 HttpClient 模拟异步发送（http/https）请求
 *
 * @author Lusifer
 * @version V1.0.0
...
public class HttpAsyncClientUtil {
	...
	public static class AsyncHandler implements IHandler {
		...
		public Object failed(Exception e) {
			logger.error("{} --失败了-- {} -- {}", Thread.currentThread().getName(), e.getClass().getName(), e.getMessage());
			return null;
		}

		@Override
		public Object completed(String respBody) {
			logger.debug("{} --获取内容-- {}", Thread.currentThread().getName(), respBody);
			return null;
		}

		@Override
		public Object cancelled() {
			logger.debug("{} --取消了", Thread.currentThread().getName());
			return null;
		}
	}
